...
The maintained-bar check runs weekly, and its latest result is here: [![Maintained check](https://github.com/jimy-r/awesome-agent-workspaces/actions/workflows/maintained-check.yml/badge.svg)](https://github.com/jimy-r/awesome-agent-workspaces/actions/workflows/maintained-check.yml). It reads each entry's last push date and keeps a standing review issue's body current every run, clean or not. Dead links and inactive projects are pruned by CI and by review.
